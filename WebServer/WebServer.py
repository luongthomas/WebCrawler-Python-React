...
def index():
	...
	maxPages = request.cookies.get('maxPages')
	searchType = request.cookies.get('searchType')
	parameters = [url, keyword, maxPages, searchType]
	# use cookies.get(key) instead of cookies[key] to not get a
	# KeyError if the cookie is missing
	print("\nHello, your cookie has been received")
	print("Your parameters are in the format of: ")
	print("[url, keyword, maxPages, searchType]")
	print(parameters)
	print("\n")
	if url:
		return url
	else:
		return "No url has been set in cookie"
...
def startSpider():
	...
	url = request.cookies.get('url')
	keyword = request.cookies.get('keyword')
	maxPages = int(request.cookies.get('maxPages'))
	searchType = request.cookies.get('searchType')
	parameters = [url, keyword, maxPages, searchType]
	# use cookies.get(key) instead of cookies[key] to not get a
	# KeyError if the cookie is missing
	...
	if searchType == "BFS":
		print("Spider is starting a breadth search from url: " + str(url) + " and searching for keyword: " + str(keyword) + " with " + str(maxPages) + " max pages.\n")
		traversalDict = breadthSpider(url, keyword, maxPages)
	elif searchType == "DFS":
